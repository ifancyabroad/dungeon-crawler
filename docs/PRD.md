...
| **Action**     | A player intent sent from the client to the server. Currently `move`, `attack` (each with a cardinal direction), `use_skill` (skillId + optional target), `wait` (pass turn), `select_skill_choice`, `reroll_skill_choice`, `pickup_item`, and `leave_loot`. Actions are the only input the server accepts. |
